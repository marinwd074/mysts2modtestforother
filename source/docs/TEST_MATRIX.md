...
- MP-1 Advisor：`READY_FOR_VALIDATION`（静态合同/Release 已通过；首轮实机 Smoke 已完成连接与战斗入口，但在远端 `BURNING_BLOOD` hook 处按合同 fail closed，尚无 `SEARCH_COMPLETE`），默认仍是 Probe，只有 `COMBATSOLVER_MULTIPLAYER_MODE=advisor` 显式 opt-in；MP-2 Safe Execute：`BLOCKED`。
