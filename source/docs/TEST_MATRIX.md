...
- Multiplayer Local Cross-Turn 当前源码合同：`MultiplayerLocalCrossTurnChecks` 11 项通过，覆盖策略边界、共享 `Shuffle RNG` 未来回合停止点、T1→T2、T1→T2→T3 本地投影、拒绝队友动作、精确 continuation、远端公开变化/WorldVersion 拒绝、目标移除身份变化、当前回合执行授权边界和 T3 可出牌平局选择。Release 构建 0 errors（保留 2 条既有 `CS9113`）。
