...
- Multiplayer Local Cross-Turn T3 空推荐修复：最终排序在多人本地跨回合模式下，局部质量相同/等价时优先保留当前回合 `PlayCard`，避免 `ActionCount` 把可执行牌路线压成仅 `EndTurn`；`current_turn_card` 诊断已加入，Release 构建和本地合同检查通过，实机复跑 `PENDING`。
