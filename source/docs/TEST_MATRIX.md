...
- Multiplayer Local Cross-Turn 当前源码合同：`MultiplayerLocalCrossTurnChecks` 9 项通过，覆盖策略边界、T1→T2、T1→T2→T3 本地投影、拒绝队友动作、精确 continuation、远端公开变化/WorldVersion 拒绝、目标移除身份变化和当前回合执行授权边界。Release 构建 0 errors（保留 2 条既有 `CS9113`）。
- Multiplayer Continuation Validation 修复（`94c6497`）：T2 continuation 校验前执行 fresh Probe；严格 WorldVersion `>` 门禁不变；拒绝路径覆盖 cached turn 缺失、本地状态差异、WorldVersion 未推进及远端公开差异，并保证 local exact + multiplayer reject 正常 fresh replan、不因空 diff 越界。Release 构建 0 errors，9 项 Local Cross-Turn 合同通过。
- Multiplayer Local Cross-Turn 实机：修复后的客户端已完成 T1→T2→T3→T4，T2 自动执行 3 张牌且无初始化失败/越界异常；T1→T2 因 `remote_public_mismatch`、T2→T3 因 `local_state_mismatch` 安全拒绝旧 continuation 并 fresh search。无新增远端公开变化的 T3→T4 已出现 `SEARCH_REUSED`、`MP_LOCAL_XTURN_CONTINUATION_REUSED`，并由新 `request_id=5` 建立 `new_authorization=true` 后执行 T4 原生牌；因此 X1 的精确复用/新授权机制已有实机证据，独立 X2 队友主动改变公开状态后的 Fresh Probe/Search 仍 `PENDING`。
- Multiplayer Local Cross-Turn T3 空推荐修复：多人本地跨回合的成员内排序和 Beam portfolio 结果比较在局部质量相同/等价时均优先保留当前回合 `PlayCard`，避免 `ActionCount` 或组合成员先后把可执行牌路线压成仅 `EndTurn`；`current_turn_card` 诊断已加入，Release 构建和本地合同检查通过，实机复跑 `PENDING`。
