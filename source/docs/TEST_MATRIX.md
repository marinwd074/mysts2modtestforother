...
- Multiplayer Carry Ranking v1 合同：新增 `MultiplayerCarryRankingChecks` 9 项通过，覆盖无远端 neutral、等风险保持 baseline、明确公开威胁移除、低远端有效生命风险、Unknown 目标 neutral、远端私有不可见和 WorldVersion/public fingerprint 变化重建上下文；纯 evaluator 不读取 Runtime/UI/live state；第 9 项固定原版 AttackIntent→AllPlayers、非攻击/第三方→Unknown 的 fail-closed 目标分类合同。
...
- Carry Ranking R1/R2 validator：新增只读 `validate-carry-ranking-results.ps1` 与 PASS/UNVERIFIED/FAIL 合成自测；R1 检查 runtime root 的机器可读 threat counts，R2 只在选中路线实际 `carryPreference>0` 且移除已证明威胁时 PASS。无正向候选的正常战斗为 `UNVERIFIED`，不误报失败。
