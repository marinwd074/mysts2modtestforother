...
- Multiplayer Carry Ranking v1 当前实现：公开上下文只在显式 Advisor/Safe Execute 的 main-thread root capture 中建立，最终排序只在既有本地安全/资源/敌方生命键之后作 tie-break；单人和默认 Probe 不启用；runtime capture 仅对原版怪物公开 AttackIntent 建立 `AllPlayers` threat，R1/R2 尚未作为实机证据收口。
