...
- Multiplayer Carry Ranking v1 合同：新增 `MultiplayerCarryRankingChecks` 11 项通过，覆盖无远端 neutral、等风险保持 baseline、明确公开威胁移除、低远端有效生命风险、Unknown 目标 neutral、远端私有不可见和 WorldVersion/public fingerprint 变化重建上下文；纯 evaluator 不读取 Runtime/UI/live state；第 9 项固定原版 AttackIntent→AllPlayers、非攻击/第三方→Unknown 的 fail-closed 目标分类合同；第 10 项固定 Carry 只有在完整 pre-carry 质量键相同、preference 更高且移除 Carry 后赢家会改变时才是决定性 tie-break；第 11 项固定威胁移除窗口只包含当前 startTurn 的 EndTurn 前动作，未来回合动作不能清除当前公开威胁。
- Multiplayer Carry Ranking v1 当前实现：公开上下文只在显式 Advisor/Safe Execute 的 main-thread root capture 中建立，最终排序只在既有本地安全/资源/敌方生命键之后作 tie-break；单人和默认 Probe 不启用；runtime capture 仅对原版怪物公开 AttackIntent 建立 `AllPlayers` threat。R1 已真实 PASS：`SLIMES_WEAK` root 捕获 1 名远端、3 敌人、2 个 `AllPlayers` threat、1 个 Unknown，`remote_private=false`。R2 保持 `UNVERIFIED`，但 current-turn 窗口修复已真实 PASS：`f54506c3` 本机 Release 0 errors/2 warnings、Carry contracts 11/11、validator self-test PASS；`carryWindow=current_turn_pre_end`、`carryObservationActionCount=3` 与 first EndTurn index 3 一致，`futureOnlyKillCredited=false`。两次 SLIMES fixture 均因 threat durability=15 而当前回合不可形成等价 lethal tie，因此不继续人工刷 R2。
- Carry Ranking R1/R2 validator：新增只读 `validate-carry-ranking-results.ps1` 与 PASS/UNVERIFIED/FAIL 合成自测；R1 检查 runtime root 的机器可读 threat counts，R2 只在选中路线实际 `carryPreference>0`、移除已证明威胁且 `carryDecisive=true` 时 PASS。无正向候选的正常战斗为 `UNVERIFIED`，不误报失败。
- Multiplayer Local Cross-Turn 当前源码合同：`MultiplayerLocalCrossTurnChecks` 12 项通过，覆盖策略边界、共享 `Shuffle RNG` 未来回合停止点、T1→T2、T1→T2→T3 本地投影、拒绝队友动作、精确 continuation、远端公开变化/WorldVersion 拒绝、目标移除身份变化、当前回合执行授权边界和 T3 可出牌平局选择。Release 构建 0 errors（保留 2 条既有 `CS9113`）。
...
- Multiplayer Local Cross-Turn Shared-RNG Shuffle Boundary 实机：基线 `22050da` 中，T1 route 保存 1 个 T2 continuation，T1→T2 以 `remote_public_soft_reuse` 成功复用；T2 EndTurn 后首次未来 shuffle 使 `continuation_pending=false`，真实 T3 从 live state Fresh Search，`cross_turn_reuse=false`，无 T3 `local_state_mismatch`、旧 future action 入队或 `SEARCH_SETUP_FAILURE`。摘要见 `multiplayer/evidence/local-cross-turn-shuffle-boundary-smoke-2026-09-20.json`。
- Multiplayer Local Cross-Turn missing-continuation 调度：运行时现直接消费 `MultiplayerContinuationScheduleDecision`，第 12 项 CI 合同覆盖远端等待时保留 route、本地 Play 且当前 turn 缺失时 Fresh Search、并清除 Awaiting/source ownership。自然 Host/Client `MP_LOCAL_XTURN_CONTINUATION_MISSING` marker 尚未单独捕获，但不再作为核心状态转换的唯一验证手段。
- Missing-continuation 本机编译：`91ed6da4` 在真实 STS2/RitsuLib `0.107.1` 环境 Release 0 errors / 2 warnings，`MultiplayerLocalCrossTurnChecks` 12/12 PASS；无需启动游戏。
- Multiplayer Local Cross-Turn T3 tie-break 与 EndTurn 边界：T3 可执行牌优先的运行复测 `PASS`；真正 EndTurn-only 允许性与 missing-continuation 调度均纳入当前 `MultiplayerLocalCrossTurnChecks 12/12 PASS`。自然 `MP_LOCAL_XTURN_CONTINUATION_MISSING` marker 只作为可选额外运行证据。
