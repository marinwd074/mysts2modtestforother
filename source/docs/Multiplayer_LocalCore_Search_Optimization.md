...
日期：2026-09-26；审查基线：`fe966f5`。本文是待实施方案，不是性能测试结果。本轮只核对当前源码并编写文档，未构建、未运行游戏。后续 GPT 开工必须刷新 HEAD、核对下列符号，已经存在的能力不得重复实现。
...
本次文档只做路径检查与 `git diff --check`，不构建、不启动游戏、不复制 DLL。下列是未来实现用例；每次选择受影响的最小集合，默认一次不超过 10 项，扩大范围先按用户要求取得批准。
