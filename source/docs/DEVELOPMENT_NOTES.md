...
- Runtime 默认 `MultiplayerProbe`；`COMBATSOLVER_MULTIPLAYER_MODE=advisor` 启用只读 Advisor，显式 `safe-execute` 才启用保守 Safe Execute，默认安装仍不升级能力。当前边界包括 `SolverPerspective`、local-player-only root contracts、schema v2 Probe 证据和 Lab-only flush 策略；重连后远端私有药水库存不可见时保持 fail-closed。
- MP-2A 单动作边界同时提供正式显式 opt-in 与受控 Lab 证据入口：`safe-execute` 只接受精确 token；`safe-execute-lab` 仍要求 Lab Probe evidence 和匹配的 `ClientCombatSolver` ownership/profile marker。2026-09-20 正式 token Host/Client Smoke 已通过，部署一次最多取 1 张本地普通安全牌，并记录原生 `PlayCardAction`、WorldVersion 失效和重新搜索所需证据；自动 EndTurn、药水、选择、Replay、队友目标、Full Auto、Instant 和连续多牌仍关闭。
