...
- MP-2A 单动作边界已具备受控 Lab 入口：`safe-execute-lab` 只有在 Lab 注入 Probe evidence 且当前目录包含匹配的 `ClientCombatSolver` ownership/profile marker 时才授予 Safe Execute；正式 `safe-execute` token 明确不接受。部署一次最多取 1 张本地普通安全牌，并记录原生 `PlayCardAction`、WorldVersion 失效和重新搜索所需证据；自动 EndTurn、药水、选择、Replay、队友目标、Full Auto、Instant 和连续多牌仍关闭。
