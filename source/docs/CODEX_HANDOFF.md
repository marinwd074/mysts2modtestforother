...
- `MP_LOCAL_XTURN_CONTINUATION_MISSING` 的专用运行夹具本轮未触发（`continuation_missing=0`），因此该分支只登记为 source contract PASS、runtime `UNVERIFIED`，不宣称已完成该项实机验收。有效 EndTurn-only 仍由 11 项 Local Cross-Turn contract 保持允许。
