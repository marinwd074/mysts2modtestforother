...
- **阶段校正（2026-09-23）**：按冻结计划，P2 是候选保留，P3 才是多情景复评。此前已实现的 Shadow 概率质量 / 一步 chance-node 保留为 P3 预备代码，不继续扩展。P2 已加入固定预算四路保护：LowTeamLoss / TeamSafety / FastLethal / Growth；它们只占现有 Beam 槽，不增加 Beam/时间预算。主搜索的 StateKey representative compression、transposition dominance 与 heuristic `MultiObjectiveDominates` 已在代码注释和 `docs/P2_CANDIDATE_RETENTION.md` 中明确区分，不再把启发式 Pareto 描述成严格等价去重。
