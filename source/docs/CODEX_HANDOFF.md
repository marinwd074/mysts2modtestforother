...
- T3 空推荐已定位：搜索存在当前回合出牌候选，但最终路线在局部质量相同/等价时被 `ActionCount` 短路线 tie-break 选成 `T3:EndTurn`。多人本地跨回合最终排序已加入“当前回合至少一张牌优先”的平局规则，并记录 `current_turn_card` 诊断；Release 与 8 项合同检查已通过，需用新构建复跑 T3 实机确认。
