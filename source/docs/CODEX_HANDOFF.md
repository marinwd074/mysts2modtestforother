...
- **P0/P1 pinned runtime 检查点（2026-09-23，本轮停止处）**：新增 `tools/P0P1PinnedHarness` 并接入 `.github/workflows/pinned-release-build.yml`，直接使用 pinned 0.107.1 游戏 DLL、RitsuLib 兼容包和生产搜索/continuation 代码；主项目、U2 harness、P0/P1 harness 均已达到 0 warnings / 0 errors。P0 manifest 17 PASS，classifier 5 PASS，U2 degenerate 继续 PASS。Runtime 首次暴露出旧 P0 fixture 中 `SOULBOUND` 在 0.107.1 `ModelDb.AllCards` 不存在，已将固定 fixture 与 harness 同步修为实际存在的 Necrobinder `DEATH_MARCH`，其余固定条件保持 NECROBINDER / A10 / PHROG_PARASITE_ELITE / GENERATED-COMBAT-001。
...
- **P0 SP-REGRESSION 暂不计 PASS**：严格沿用 Medium / Smart / fixed 5000 ms / DOP1 后，在 GitHub pinned runner 上结果为 `Boundary=TimeLimit`，首动作 `STRIKE_NECROBINDER`，22 actions，预计战损 4，final HP 62，enemy HP 7，1323 expanded，5 continuations。禁止通过提高时间/Beam 掩盖；下轮应先核对原 unattended Search launcher 是否实际启用 BeamWidthPortfolio，再判断这是 harness 口径差异还是生产搜索性能/完成性回归。
- **P1 runtime 已跑到生产搜索，但暂不计完整 PASS**：AdaptiveLethalTempo 与 MinimizeTeamLoss 都找到了相同 T5 胜利路线（final HP 66、enemy HP 0、team loss 0、all players alive），但两次均以 `Boundary=TimeLimit` 返回；Adaptive 3005 expanded / 16001 transitions，MinimizeTeamLoss 3946 / 20830。独立生产 `MultiplayerCombatObjectiveMath` 断言已通过：Adaptive 的低耐久终局中“更快但多 4% team loss”优于慢两回合路线；`AllPlayersAlive` 仍是硬边界。下一轮从“确认 P0/P1 原 launcher 的 portfolio/预算语义”继续，不先改算法。
