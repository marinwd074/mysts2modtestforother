...
- Multiplayer Lab snapshot：已改为 schema 2 的持久 base-game snapshot + profile overlay 增量同步。游戏版本/底座变化、底座完整性失败或旧 schema 才全量重建；CombatSolver/RitsuLib 变化分别只更新各自 payload，HostVanilla 不因 CombatSolver 构建变化重建。`syncMode` 会记录同步结果；ownership、no-reparse-point、运行中禁止覆盖和正式证据隔离合同保持不变。
