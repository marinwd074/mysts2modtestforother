...
- **P0 SP 的搜索语义未发现当前版本回归，但原 5000 ms 性能门仍未封口**：已确认原 unattended Search 与 pinned harness 口径一致——Medium / Smart / fixed 5000 ms / DOP1，且 BeamWidthPortfolio 默认开启并共享整个 5000 ms 请求预算。历史 `380b0801` 与当前版本在正式 5 秒口径下都命中 `TimeLimit`，因此仍分类为 `P0_BASELINE_AB INCONCLUSIVE_TIME_BOUNDARY`，不能把该 wall-clock 样本计为 P0 SP PASS。新增的确定性 A/B 不替代这条性能门：同一 root、Beam 60、DOP1、单成员、关闭 wall-clock 截断、固定 1200 expanded nodes 时，历史与当前均为首动作 `PROWESS`、战损 0、final HP 66、enemy HP 4、1200 expanded，分类 `P0_FIXED_WORK OBSERVED_EQUIVALENT`。因此当前没有证据支持“P0 搜索质量回归”；后续若处理 P0，只需单独研究 5 秒性能/完成性，不再重复核心语义 A/B，也不得扩大正式预算冒充 PASS。
