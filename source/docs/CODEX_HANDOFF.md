...
- **U2 搜索共核代码/合同阶段已推进（2026-09-23）**：SinglePlayerFullRoute 与 MultiplayerLocalCrossTurn 现在不仅共用 Novelty/Growth/Relic/长期收益，也共用配置的本地药水搜索策略；CaptureSearchPolicy 不再用 CanUsePotionsAutomatically 削掉多人搜索候选，自动药水执行权限仍保持关闭。SearchPolicySnapshot 新增 UseMultiplayerTeamObjective，把 route mechanics 与团队目标解耦；生产真实多人仍由 capabilities.IsMultiplayer 开启团队目标，单人保持旧目标。BeamRetentionPolicy 与 FinalPlanOrdering 已改读显式 objective mode。OfflineSearchHarness 新增 --route-policy，可在同一单人根上仅切 SinglePlayerFullRoute / MultiplayerLocalCrossTurn 并保持同一目标；tools/test-u2-degenerate-equivalence.ps1 比较首动作、终局值与固定 tie-break 完整动作序列。tools/test-u2-search-kernel.ps1 已接入总合同。详细状态见 docs/U2_SEARCH_KERNEL.md。**实际 differential 尚未运行，因此 U2 仍不能标 COMPLETE。**
...
- **当前 runtime blocker 仍是 U1 smoke，但不再阻断用户明确要求的 U2 代码/合同推进。** U1 必须用新 DLL 实测三类样例后才能把安全执行链标为稳定：①重锤 + 真实 Choice/烙印 + 后续牌；②连续祭品抽出并继续使用后续牌；③队友在两张本地动作之间插入一个可读变化，确认 `U1_PRE_ACTION_PROBE` 在下一张 native submit 前使旧 session 失效。另保留 cancellation/late callback 不重复提交检查。最小证据链：`U1_PRE_ACTION_PROBE → U1_EXPECTED_POST_STATE → NATIVE_ACTION_CAPTURED → DEPLOY_ACTION_COMPLETE → U1_POST_STATE_COMPARE → MP2B_ACTION_RECONCILED`。U2 的同根离线 differential 也是独立 UNVERIFIED gate。
