...
- **P2 已闭环并通过 CI；当前进入 P3 多情景复评（2026-09-23）**。P3 使用 Aggressive / Defensive / Conserve / NoAction 四类压力情景，不把通用行为 prior 当真实概率。只对 P1/P2 前 4 个不同当前动作组做 final-only 复评，每组最多 4 个情景代表；不增加主 Beam、搜索深度或部署权限。跨情景以同一 `CurrentTurnDecisionKey` 聚合，先看全员存活，再看全情景斩杀、最坏战损、最差队员和平均战损。概率加权 chance 代码继续关闭，等待后续真实队友行为校准。
