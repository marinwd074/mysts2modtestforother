...
- **P0 SP 的搜索语义未发现当前版本回归，但原 5000 ms 性能门仍未封口**：已确认原 unattended Search 与 pinned harness 口径一致——Medium / Smart / fixed 5000 ms / DOP1，BeamWidthPortfolio 默认开启并共享整个 5000 ms 请求预算。历史 `380b0801` 与当前版本在正式 5 秒口径下都命中 `TimeLimit`，分类 `P0_BASELINE_AB INCONCLUSIVE_TIME_BOUNDARY`。确定性 A/B 不替代该性能门：同一 root、Beam 60、DOP1、单成员、固定 1200 expanded nodes、关闭 wall-clock 截断时，历史与当前完全一致——首动作 `PROWESS`、战损 0、final HP 66、enemy HP 4、1200 expanded，`P0_FIXED_WORK OBSERVED_EQUIVALENT`。进一步保持 Medium / Smart / 5000 ms / DOP1 与完整 `CombatSearchCoordinator`，仅关闭 BeamWidthPortfolio 的 timed 单成员诊断中，历史和当前仍都 `TimeLimit`，且都已找到 enemy HP 0 路线，分类 `P0_SINGLE_MEMBER_TIMED BOTH_TIME_BOUNDARY`（历史 4687 expanded，当前 4032）。因此 portfolio 不是该 5 秒边界的根因，也没有当前版本搜索质量回归证据；P0 剩余项只应作为该固定场景的 5 秒性能/完成性问题处理，不再重复语义 A/B，也不得扩大正式预算冒充 PASS。
