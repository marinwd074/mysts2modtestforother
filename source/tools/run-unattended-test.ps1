...
$script:headlessProcessOutputStates = @{}

function Start-HeadlessGameProcess(
    [string]$Executable,
    ...
    foreach ($argument in $Arguments) {
        # ProcessStartInfo.ArgumentList preserves argv boundaries; Start-Process
        # -ArgumentList joins values into a command line before starting.
    ...
    if (-not $process.Start()) {
        $process.Dispose()
        throw "Could not start headless game process: $Executable"
    }
    $streams = @()
    $tasks = @()
    try {
        ...
        if ($tasks.Count -gt 0) {
            $script:headlessProcessOutputStates[$process.Id] = [PSCustomObject]@{
                Process = $process
                Streams = $streams
                Tasks = $tasks
            }
...
function Complete-HeadlessGameProcessOutput([Diagnostics.Process]$Process) {
    if ($null -eq $Process) { return }
    $state = $script:headlessProcessOutputStates[$Process.Id]
    ...
    try {
        ...
        [void]$script:headlessProcessOutputStates.Remove($Process.Id)
    }
}
...
function Stop-ClaimedProcessAndRemoveDependency(
    ...
    # Marker recovery and the launcher both retain this exact Process object's
    ...
    Complete-HeadlessGameProcessOutput $TestProcess
...
if (-not $reusedProcess) {
    ...
    try {
        Assert-LauncherNotCancelled
        $process = Start-HeadlessGameProcess `
            -Executable $gameExe `
            ...
            -RedirectStandardOutputPath (Join-Path $headlessRoot 'native-stdout.log') `
            -RedirectStandardErrorPath (Join-Path $headlessRoot 'native-stderr.log')
