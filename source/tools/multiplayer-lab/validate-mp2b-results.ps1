...
if ($capability.Count -eq 1) {
    if ($capability[0].Text -match 'action_limit=route_bounded\b') {
        ...
        # Historical logs advertised a fixed capability ceiling. Keep them readable as
        # evidence, but current production capability is route-bounded and reports its
        # concrete finite capacity only at MP2B_DEPLOY_START.
