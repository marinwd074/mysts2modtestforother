...
Assert-Contains $controller 'capabilities.IsMultiplayer && settings.UseMultiplayerPrediction' 'production multiplayer prediction is gated by the persisted master switch'
Assert-Contains $controller 'SearchRoutePolicy.MultiplayerSinglePlayerCore' 'default multiplayer route uses the single-player search core'
Assert-Contains $controller 'capabilities.CanPlanLocalCrossTurn && !useMultiplayerPrediction' 'multiplayer-only route semantics require the explicit option'
Assert-Contains $controller 'UseMultiplayerTeamObjective = useMultiplayerPrediction' 'master switch gates team objective'
Assert-Contains $controller 'UseMultiplayerTeammateForecast = useMultiplayerPrediction' 'master switch gates teammate forecast'
Assert-Contains $controller 'UseMultiplayerScenarioReevaluation = useMultiplayerPrediction' 'master switch gates scenario reranking'
Assert-Contains $controller '? "team_prediction"' 'diagnostics expose enabled prediction mode'
Assert-Contains $controller ': "single_player_core"' 'diagnostics expose default single-player search-core mode'
...
Assert-Contains $contracts 'SearchRoutePolicy.MultiplayerSinglePlayerCore' 'multiplayer single-player-core route contract'
Assert-Contains $contracts 'SearchRoutePolicy.MultiplayerLocalCrossTurn' 'multiplayer local-cross-turn contract'
Assert-Contains $contracts 'HasLocalCrossTurnProjection(' 'multiplayer single-core keeps runtime continuation projection'
Assert-Contains $contracts 'or SearchRoutePolicy.MultiplayerSinglePlayerCore' 'single-core multiplayer route receives full-search/cache parity'
