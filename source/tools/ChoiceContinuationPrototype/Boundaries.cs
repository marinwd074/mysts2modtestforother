...
{
    private static void VerifyChoicePrototypeBoundaries(CombatPredictionSimulator parent, Player player,
        Creature target, SolverDisplayNames names, Func<CombatPredictionSimulator, string> stamp)
    {
        ...
        ChoicePrototypeCheckpoint? Capture(CombatPredictionSimulator basis)
        {
            var seed = basis.Fork();
            return seed.PauseChoicePrototype(Card(seed), target, new HashSet<uint>());
        }
        (CombatPredictionSimulator Sim, string Path) Driver(CombatPredictionSimulator basis, IReadOnlyList<PlanCardChoice> plans)
        {
            ...
            if (plans.Count == 1)
            {
                using ChoicePrototypeCheckpoint? checkpoint = Capture(basis);
                if (checkpoint != null)
                {
        ...
        (ChoicePrototypeCheckpoint disposed, WeakReference<CombatPredictionSimulator> seed) =
            MakeDisposedChoicePrototype(parent, player, target);
    ...
    private static (ChoicePrototypeCheckpoint, WeakReference<CombatPredictionSimulator>) MakeDisposedChoicePrototype(
        CombatPredictionSimulator parent, Player player, Creature target)
    {
        ...
        var checkpoint = seed.PauseChoicePrototype(dagger, target, new HashSet<uint>())
        ...
        catch (InvalidOperationException e) when (e.Message == "Suspended prototype seed requires its owning checkpoint fork.") { }
        checkpoint.Dispose();
