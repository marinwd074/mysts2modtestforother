...
{
    private async Task RunPriorityChoicePrototypeAsync(CombatState live, Player player)
    {
        string cardId = _request.ScenarioId switch
        {
            "CHOICE-PROTOTYPE-ACROBATICS" => "ACROBATICS",
            "CHOICE-PROTOTYPE-PREPARED" => "PREPARED",
            _ => throw new InvalidOperationException("Unknown priority-card prototype scenario."),
        };
        ...
        foreach (int upgrade in new[] { 0, 1 })
        {
            ...
            ChoicePrototypeCheckpoint Pause(CombatPredictionSimulator basis)
            {
                using IDisposable isolation = SimulationNotificationIsolation.Enter();
                var seed = basis.Fork();
                return seed.PauseChoicePrototype(Card(seed), null, new HashSet<uint>())
                    ?? throw new InvalidOperationException(cardId + " rejected: " + seed.ChoicePrototypeRejection);
            }
            ...
            PlanCardChoice[] Choices(CombatPredictionSimulator basis)
            {
                ...
                if (discovery.ChoicePrototypeRejection != "own choice not reached")
            ...
            using (var checkpoint = Pause(parent))
            {
                ...
                for (int i = choices.Length - 1; i >= 0; i--)
                {
                    ...
                    AssertChoicePrototypeIdentity(sim, player, cardId);
            ...
            _completedChecks.Add($"ChoicePrototype:{cardId}+{upgrade}:all={choices.Length}:draw={draws}:discard={discards}:full-state-history-rng:shuffle:DOP2:nested-fallback");

            List<object> samples = [];
            using (SimulationNotificationIsolation.Enter())
            {
                foreach (int branches in new[] { 2, 4, 8, 32 }.Where(n => n <= choices.Length))
                {
                    void Batch(bool resume)
                    {
                        ...
                        {
                            _ = Replay(parent, null);
                            for (int i = 0; i < branches; i++) _ = Replay(parent, [choices[i]]);
                        }
                    }
                    // Time-bounded warm-up targets delayed tiered compilation seen in the dagger run.
                    ...
                    foreach (bool resume in new[] { false, true, true, false, false, true, true, false })
                    {
                        ...
                        for (int repeat = 0; repeat < repetitions; repeat++) Batch(resume);
                        double ms = Stopwatch.GetElapsedTime(ticks).TotalMilliseconds;
                        samples.Add(new { branches, resume, repetitions, ms, bytes = GC.GetAllocatedBytesForCurrentThread() - bytes });
                    }
                }
            }
            File.WriteAllText(Path.Combine(_request.EvidenceDirectory!, $"choice-prototype-{cardId}-{upgrade}.json"), JsonSerializer.Serialize(
                new { source = "1ef4601", cardId, title, upgrade, draws, discards, choices = choices.Length, samples }, new JsonSerializerOptions { WriteIndented = true }));
            ...
            using NativeChoiceSession session = NativeChoiceRuntime.Begin(live, player, "test:priority-choice-prototype");
            ...
            _completedChecks.Add($"ChoicePrototype:{cardId}+{upgrade}:{title}:native-full-continuation:fixed-work-ABBA");
        }
    }
}
