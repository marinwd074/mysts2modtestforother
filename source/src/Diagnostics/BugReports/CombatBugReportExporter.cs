...
internal static class CombatBugReportExporter
{
    ...
    private const long MaximumCapturedGameLogBytes = 8L * 1024 * 1024;
    ...
    public static Task<string> ExportCurrentAsync(string? outputDirectory = null, string? playerDescription = null, string? submissionId = null)
    {
        ...
        {
            Entry.Logger.Warn($"[CombatSolver/Diagnostics] GAME_LOG_SYNC_FAILED error={error}");
        }
    ...
    private static void WriteGameLogs(ZipArchive archive)
    {
        string path = Path.Combine(CombatBugReportPaths.GameLogsDirectory, "godot.log");
        if (File.Exists(path))
            AddFile(archive, path, "diagnostics/game-logs/godot.log", MaximumCapturedGameLogBytes);
    }
    ...
    private static string DefaultExportDirectory()
    {
        ...
        return CombatBugReportPaths.RootDirectory;
    }
