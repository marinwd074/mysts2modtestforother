...
internal sealed partial class CombatBeamSolver
{
        ...
        IReadOnlySet<uint> processedEnemyDeaths)
    {
        ...
        if (combat.HasUnsupportedKnowledgeDemonMultiplayerChoice)
            boundary = SearchBoundaryReason.UnsupportedEffect;
        else if (boundary == SearchBoundaryReason.None && combat.HasPendingChoice)
        ...
        for (int cardIndex = 0; cardIndex < handCards.Count; cardIndex++)
        {
            ...
            if (!card.Preview.ShouldRetainThisTurn)
                continue;
            // Enumerable.Sum(int) uses checked accumulation; preserve overflow behavior.
        ...
        bool hasBlockDamagePayoff = false;
        for (int cardIndex = 0; cardIndex < liveCards.Count; cardIndex++)
        {
