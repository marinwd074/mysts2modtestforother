...
{
#if STS2_01071
    // 0.107.1 exposes GetResultPileTypeForCardPlay instead of the later
    // GetResultLocationForCardPlay/CardLocation API. Keep this path fully
    // local so the compatibility build does not reflect a missing method.
        ...
        PredictedCard card)
    {
        ...
        var context = new CardResultLocationMirrorContext
        {
            Simulator = simulator,
            Card = card,
            BaseResult = result
        ...
        if (preview is ParticleWall)
        {
            result = HandleParticleWall(preview, context);
        }
        else if (preview is ShiningStrike)
        {
            result = HandleShiningStrike(preview, context);
        }
