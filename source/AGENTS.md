...
- 发布包、完整日志、问题包、Profiler、`.local/`、`bin/`、`obj/` 和 `.godot/` 不进入 `source/` 源码目录；新产生的运行日志、Mod 使用日志和报错按本项目根目录 `AGENTS.md` 的运行证据规则单独存入仓库根目录 `runtime-evidence/`，整理元数据后提交并推送，不与源码混在同一目录。
