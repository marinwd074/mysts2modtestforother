...
> **当前工作项：** `AEONGLASS_BOSS`、`SOUL_NEXUS_ELITE`、`BYGONE_EFFIGY_ELITE` 三份新问题包已闭环；`CombatSolver_architecture_optimization_plan` 的 BeamRetentionPolicy Choice、Potion、Mutation、Cycle、CrossTurn 物理拆分、`AfterBlockBroken` Hook 参数兼容边界及回合准备补丁目标参数边界已完成，继续后续兼容 API 审计。
...
- 普通源码、测试和文档改动直接提交到当前任务分支；完成相称验证后遵循本项目根目录 `AGENTS.md` 的 GitHub 同步规则，将本任务提交推送到已配置的 GitHub 远端。发包时机遵循第 9 节的批次状态和用户口令；版本标签、创意工坊、干净安装和完整门禁仍需按用户口令，不由普通改动自动触发。
...
- `CombatBeamSolver.BeamRetentionPolicy.cs`：状态去重、Beam 排名、多样性通道和小型 Pareto；继续拥有嵌套 `BeamRetentionPolicy` 的主体策略。
...
- `CombatBeamSolver.BeamRetentionPolicy.Potion.cs`：最终政策资格记录/比较、药水配额、药水谱系分组和 `UsesPotion` 辅助；只声明同一嵌套 `BeamRetentionPolicy` 的 partial，不改变候选选择或配额调用顺序。
- `CombatBeamSolver.BeamRetentionPolicy.Mutation.cs`：有序变异碰撞的激活、续接、admission、observation 与 key-policy 校验；只声明同一嵌套 `BeamRetentionPolicy` 的 partial。预算常量、lineage、lease/ledger 和最终提交仍归 `OrderedMutationRetention.cs`，不在这里复制。
...
- 普通开发完成后直接提交当前任务改动并按本项目根目录 `AGENTS.md` 的 GitHub 同步规则推送；“干净提交”表示显式暂存本任务文件、保留用户其他改动并排除构建产物、发布包和暂存内容，不表示清空工作区。没有新改动但已有本地提交领先远端时，直接推送，不创建空提交。
- 版本号、manifest、发布 ZIP 和创意工坊上传使用 `release-gate`；干净安装和 Steam 发布验收只在用户明确要求完整发布门禁时执行。创意工坊上传成功本身不触发重新打开页面、下载订阅内容或重复核对远端版本。
- 发布包、完整日志、问题包、Profiler、`.local/`、`bin/`、`obj/` 和 `.godot/` 不进入 `source/` 源码目录；只有出现新的失败模式、新验证结论或已有 Bug 的新关键证据时，才按本项目根目录 `AGENTS.md` 的运行证据规则将日志、报错和问题包单独存入仓库根目录 `runtime-evidence/`，整理元数据后提交并推送；重复运行和等价失败不提交。
