...
> **当前工作项：** 按 `CombatSolver_architecture_optimization_plan` 执行 Batch 4（Deployment），已将全自动部署、动作执行、部署延迟、取消和部署引用释放移入 `SolverController.Deployment.cs`；保持 `static partial`、单程序集、单人模式和既有搜索语义。
