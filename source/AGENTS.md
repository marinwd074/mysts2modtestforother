...
- 普通源码、测试和文档改动直接提交到当前任务分支；完成相称验证后遵循全局 GitHub 同步规则，将本任务提交推送到已配置的 GitHub 远端。发包时机遵循第 9 节的批次状态和用户口令；版本标签、创意工坊、干净安装和完整门禁仍需按用户口令，不由普通改动自动触发。
...
- 普通开发完成后直接提交当前任务改动并按全局 GitHub 同步规则推送；“干净提交”表示显式暂存本任务文件、保留用户其他改动并排除构建产物、发布包和暂存内容，不表示清空工作区。没有新改动但已有本地提交领先远端时，直接推送，不创建空提交。
- 版本号、manifest、发布 ZIP 和创意工坊上传使用 `release-gate`；干净安装和 Steam 发布验收只在用户明确要求完整发布门禁时执行。创意工坊上传成功本身不触发重新打开页面、下载订阅内容或重复核对远端版本。
- 发布包、完整日志、问题包、Profiler、`.local/`、`bin/`、`obj/` 和 `.godot/` 不进入 `source/` 源码目录；新产生的运行日志、Mod 使用日志和报错按全局运行证据规则单独存入仓库根目录 `runtime-evidence/`，整理元数据后提交并推送，不与源码混在同一目录。
